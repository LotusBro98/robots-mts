...
class CartohrapherNested:
    ...
    def update_map(self, world_points, relative_points, pos, angle):
        ...
        points = transform_points(relative_points, pos, angle, (0, 0))
        world_points = self.add_scan_with_buffer(points, min_dist=0.05, promote_hits=10, world_enter_hits=2, max_candidate_age=2, ema_alpha=0.1)
...
def _cartographer_process(req_q: mp.Queue, res_q: mp.Queue):
    ...
    while True:
        ...
        res_q.put((world_points, cur_matched_pts, points, pos, angle))
...
class Cartographer:
    ...
    def _cartographer_thread(self):
        while not self._stop:
            world_points, cur_matched_pts, points, pos, angle = self._res_q.get()
            self.output_cb(world_points, cur_matched_pts, points, pos, angle)
